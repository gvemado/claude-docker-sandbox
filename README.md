...
- Creates or reuses one persistent worktree per branch, so you can run multiple containers on different branches at the same time
...
The script prompts for a branch name. If the branch already has a worktree, it reuses it. If the branch does not exist yet, it creates a new branch and a new worktree for it.
